...
export default function ReceptionistIpdBillsClient() {
  ...
  // Fetch bills (same API supports receptionist auth)
  const { data, isLoading } = useQuery<BillsResponse>({
    queryKey: ['receptionist-ipd-bills', activeTab, search, page],
    queryFn: async () => {
  ...
  const { data: admittedPatients, isLoading: loadingAdmissions } = useQuery<AdmittedPatient[]>({
    queryKey: ['receptionist-admitted-patients'],
    queryFn: async () => {
      const res = await fetch('/api/ipd-admissions?status=Admitted&limit=100')
      if (!res.ok) throw new Error('Failed to load admitted patients')
  ...
  const generateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['receptionist-ipd-bills'] })
    },
    onError: (error) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        ...
        <Button
          variant="outline"
          size="icon"
          onClick={() => queryClient.invalidateQueries({ queryKey: ['receptionist-ipd-bills'] })}
        >
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right hidden sm:table-cell">Net Payable</TableHead>
                  ...
                  <AnimatePresence>
                    {bills.map((bill) => (
                      ...
                      >
                        ...
                        <TableCell className="text-right font-mono font-semibold hidden sm:table-cell">{formatCurrency(bill.netPayable)}</TableCell>
